...
async function loadPage(locale: string) {
  return await fetchGlobal('freshman-page', validateLocale(locale))
}
...
export default async function FreshmanPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  const page = await loadPage(locale)
    ...
    <main>
      ...
      <div className="bg-white">
        {page.infoSections.map((section, i) => (
          <section key={section.id} className="my-12 sm:my-16 md:my-20">
